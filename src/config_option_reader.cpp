...
#define BOOLEAN_ARG(ARG, NAME) \
        ...
        if (option.hasValue) { \
            printf( \
                "Option '%s' did not expect a value\n", \
                option.key.c_str() \
            ); \
            return false; \
        } \
...
bool ConfigOptionReader::read(int argc, const char **argv, Config *config) {
    ...
    for (int i = 0; i < options.size(); i++) {
        ...
        if (option.key.empty()) {
            if (!config->sourceFile.empty()) {
                printf(
                    "Invalid argument at position %d: '%s'\n",
                    i + 1,
                    option.value.c_str()
                );
                return false;
            }
            config->sourceFile = option.value;
        }
